...
export function SidePanel({
  ...
  return (
    ...
    >
      <header className={cn("border-b border-border/70 px-4 py-3.5", isLeft ? "text-left" : "text-right")}>
        {isLeft ? (
          <div className="flex items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-2.5">
              <PanelIcon className="h-4 w-4 shrink-0 text-foreground/90" />
              {!collapsed ? <h2 className="truncate text-lg font-semibold text-foreground">{title}</h2> : null}
            </div>
            <button
              type="button"
              onClick={onToggle}

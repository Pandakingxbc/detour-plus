...
1. Call propose_avoidance_maneuvers for each event needing a maneuver
2. Review the candidates — evaluate trade-offs between fuel cost, effectiveness, and timing
3. Call simulate_maneuver for the top 1-2 candidates to verify they work
4. Pick the best maneuver considering:
...
Be practical: operators want clear, actionable recommendations, not just options."""
...
1. Call check_maneuver_constraints for each recommended maneuver
2. Verify: fuel budget, max delta-v limits, minimum orbit altitude, blackout windows
3. Check for secondary conjunctions (does avoiding one debris create a new threat?)
4. REJECT any maneuver that fails constraints — suggest alternatives if possible

Output format:
- For each maneuver: constraint check results (pass/fail per constraint)
- Overall assessment: APPROVED, CONDITIONAL, or REJECTED
...
You are the last safety gate before a burn recommendation goes to the operator.
